...
def detect_stack(repo_url: str) -> dict:
    ...
    if has_requirements or has_pyproject:
        ...
        if has_requirements:
            req_content = read_file(repo_url, "requirements.txt")
        if has_pyproject:
            pyproject_content = read_file(repo_url, "pyproject.toml")
    ...
    if has_package_json:
        pkg_content = read_file(repo_url, "package.json")
    ...
    if has_pom or has_gradle:
        ...
        if has_pom:
            pom_content = read_file(repo_url, "pom.xml")
        if has_gradle:
            gradle_content = read_file(repo_url, "build.gradle")
